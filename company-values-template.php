<?php
/**
 * Template Name: Company & Values Template
 *
        ...
        $output .= '<section class="innerpages content-wrap">';
            ...
            $output .= '<div class="container-wrap container-fliud">';
                $output .= '<div class="container-inner-wrap row">';
                                                        ...
                                                        if ( ! empty( $cv_tab_list ) ) {
                                                            if ( $active_tab === $cv_tab_list_id ) {
                                                                ...
                                                                foreach ( $cv_tab_list as $cv_tab_details ) {
																	...
																	$cv_tab_list_logo_id = attachment_url_to_postid($cv_tab_list_logo);
						$cv_tab_list_logo_alt = get_post_meta($cv_tab_list_logo_id, '_wp_attachment_image_alt', TRUE);
                                                                    $cv_tab_list_desc = core_isset_array( $cv_tab_details, 'description', '');

                                                                    $output .= '<div class="row core-tab-details">';
                                                                        
                                                                        $output .= '<div class="col-md-3 core-tab-logo">';
                                                                            $output .= '<img src="' . esc_url( $cv_tab_list_logo ) . '" alt="'.$cv_tab_list_logo_alt.'">';
                                                                        $output .= '</div>';
                        
                                                                        $output .= '<div class="col-md-9 core-tab-desc">';
                    ...
                    $output .= '<section class="page-section economic-sustainability-journey container">';
                        $output .= '<div class="section-wrap row">';
                            $output .= '<div class="inner-section col-md-12">';
                            
                                $output .= '<h3 class="section-title center" id="EnvironmentalSustainabilityJourney">';
                                    $output .= core_encapsulate_text(
                                            array(
                                                'text'          => esc_html( $esj_title ),
                                                'word_position' => 0,
                                                'before'        => '<span class="highlight">',
                                                'after'         => '</span>',
                                            )
                                ...
                                $output .= '<div class="section-contents center">';

                                    $output .= '<div class="section-desc">';
                                        $output .= wp_kses_post( $esj_desc );
                                    $output .= '</div>';

                                    $output .= '<a href="' . esc_url( $esj_btn_link ) . '" class="com-btn inline">';
                                        $output .= esc_html( $esj_btn_label );
                                    $output .= '</a>';
                    ...
                    $output .= '<section class="page-section full-scaled-column left economic-sustainability container-fluid" style="background-color: ' . esc_attr( $es_bg_color ) . ';">';
                        $output .= '<div class="section-wrap container">';
                    ...
                    $output .= '<section class="page-section image-background core-grid-items core-values container-fluid" style="background-image: url(' . esc_attr( $cvs_bg_image ) . '   ); --data-color: ' . esc_attr( esc_attr( $cvs_bg_color ) ) . '">';
                        $output .= '<div class="section-wrap">';
                            $output .= '<div class="inner-section row">';
                            
                                $output .= '<h3 class="section-title center" id="OurCoreValues">';
                                    $output .= core_encapsulate_text(
                                            array(
                                ...
                                $output .= '<div class="section-contents container center">';
                                        $output .= '<div class="section-desc col-md-12">';
                                        ...
                                        $output .= '<div class="grid-items">';
                                            $output .= '<div class="row">';
                                                ...
                                                foreach ( $cvs_grid_items as $key => $cvs_grid_item ) {
													...
													$icon_id = attachment_url_to_postid($cvs_grid_icon);
                                                    ...
                                                    $output .=  '<div class="' . esc_attr( $cvs_col_class ) . '">';
                                                        ...
                                                        $output .= '<div class="grid-image-wrap">';
                                                            $output .= '<img src="' . esc_url( $cvs_grid_icon ) . '" alt="'.$icon_alt.'">';
                                                        $output .= '</div>';
                    ...
                    $output .= '<section class="page-section economic-sustainability container">';
                        $output .= '<div class="section-wrap">';
                            $output .= '<div class="inner-section row">';

                        //     $unashamedly_ethical = get_field( 'unashamedly_ethical' );
                        //     $ue_title    = core_isset_array( $core_values, 'title', '' );
                        //     $ue_desc     = core_isset_array( $core_values, 'description', '' );
                        //     $ue_bg_image = core_isset_array( $core_values, 'background_image', '' );
                        // $code_of_conduct_for_individuals = get_field( 'code_of_conduct_for_individuals' );
                        //     $cocfi_title = core_isset_array( $core_values, 'title', '' );
                        //     $cocfi_list  = core_isset_array( $core_values, 'list', '' );
                        //         $cocfi_list_item = core_isset_array( $core_values, 'list_item', '' );
                        		$ue_bg_image_id = attachment_url_to_postid($ue_bg_image);
								$ue_bg_image_alt = get_post_meta($ue_bg_image_id, '_wp_attachment_image_alt', TRUE);

                                $output .= '<h3 class="section-title center" id="UnashamedlyEthical">';
                                    $output .= core_encapsulate_text(
                                            array(
                                                'text'          => esc_html( $ue_title ),
                                                'word_position' => 0,
                                                'before'        => '<span class="highlight">',
                                                'after'         => '</span>',
                                            )
                                ...
                                $output .= '<div class="details-col col-md-6">';
                                    
                                    $output .= '<div class="section-contents">';
                                        $output .= '<div class="section-desc">';
                                            $output .= wp_kses_post( $ue_desc );
                                        $output .= '</div>';
                                    $output .= '</div>';
                                    
                                $output .= '</div>';

                                $output .= '<div class="image-col col-md-6">';
                                    $output .= '<img src="' . esc_url( $ue_bg_image ) . '" alt="'.$ue_bg_image_alt.'">';
                                $output .= '</div>';
                            ...
                            if ( ! empty( $cocfi_list ) ) {
                                $output .= '<div class="inner-section row">';
                                    ...
                                    if ( ! empty( $cocfi_title ) ) {
                                        $output .= '<div class="section-sub-title-wrap col-md-12">';
                                            $output .= '<h4 class="section-sub-title">';
                                            $output .= esc_html( $cocfi_title );
                                            $output .= '</h4>';
                                        $output .= '</div>';
                                    }

                                    if ( ! empty( $cocfi_list ) ) {
                                        $output .= '<div class="section-list-wrap col-md-12">';
                                            $output .= '<div class="row">';
                                                foreach ( $cocfi_list as $key => $list ) {
                                                    $cocfi_list_item = core_isset_array( $list, 'list_item', '' );

                                                    $output .= '<div class="list-item col-md-6">';
                                                        $output .= '<i class="list-icon fa fa-chevron-right"></i>';
                                                        $output .= wp_kses_post( $cocfi_list_item );
                                                    $output .= '</div>';
                                                }
                                            $output .= '</div>';
                                        $output .= '</div>';
